...
export const config: WebdriverIO.Config = {
    ...
    beforeTest: async ()=> {
        if (driver.isAndroid){
            ...
            log.info("Start IOS Application has app bundleID: " + process.env.DEFAULT_BUNDLE_ID)
            await driver.execute('mobile: activateApp', { bundleId: process.env.DEFAULT_BUNDLE_ID });
        } else {
    ...
    afterTest: async ()=> {
        if (driver.isAndroid){
            ...
            log.info("Reset IOS Application has app bundleID: " + process.env.DEFAULT_BUNDLE_ID)
            await driver.execute('mobile: terminateApp', { bundleId: process.env.DEFAULT_BUNDLE_ID });
        } else {
