...
export async function getSpecsByConfigFileName(configFileName: string): Promise<string[]> {
    if (process.env.SPECS !== undefined && process.env.SPECS !=='') {
        return  process.env.SPECS.split(",")
    } else {
        if (configFileName.startsWith("wdio.app.mobile.ios")) { // @ts-ignore
            return  Constants.APP_MOBILE_IOS_DEFAULT_SPECS.split(',')
        } else if (configFileName.startsWith("wdio.app.mobile.android")) { // @ts-ignore
            return  Constants.APP_MOBILE_ANDROID_DEFAULT_SPECS.split(',')
        } else if (configFileName.startsWith("wdio.web")) { // @ts-ignore
            return  Constants.WEB_DEFAULT_SPECS.split(',')
        } else {
            return ["../../tests/specs/**/*.ts"]
        }
    }
}

export async function getAppByConfigFileName(configFileName: string): Promise<string> {
    if (process.env.BS_APP_NAME !== undefined) {
        return process.env.BS_APP_NAME
    } else {
        if (configFileName.startsWith("wdio.app.mobile.ios.bs")) { // @ts-ignore
            return Constants.BS_DEFAULT_IOS_DEFAULT_APP
        } else if (configFileName.startsWith("wdio.app.mobile.android.bs")) { // @ts-ignore
            return Constants.BS_DEFAULT_ANDROID_DEFAULT_APP
        } else {
            return "";
        }
    }
